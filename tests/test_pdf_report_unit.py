...
class TestNumberedCanvas:
    ...
    def test_save_single_page_with_numbering(self, tmp_path):
        """Test single page PDF still gets page numbering."""
        output_path = tmp_path / "single_page.pdf"
        ...
        styles = getSampleStyleSheet()
        story.append(Paragraph("Single Page Document", styles["Normal"]))

        doc.build(story, canvasmaker=NumberedCanvas)

        assert output_path.exists()
        # File should be valid PDF (magic number check)
        pdf_content = output_path.read_bytes()
        assert pdf_content[:4] == b"%PDF"

    def test_save_with_many_pages(self, tmp_path):
        """Test PDF with many pages gets correct numbering."""
        output_path = tmp_path / "many_page.pdf"
        ...
        # Create 10 pages
        for i in range(10):
            story.append(Paragraph(f"Page {i + 1}", styles["Normal"]))
            if i < 9:
                story.append(PageBreak())
    ...
    def test_canvas_initialization(self):
        """Test NumberedCanvas initialization."""
        from io import BytesIO

        buffer = BytesIO()
        canvas = NumberedCanvas(buffer)

        assert canvas is not None
        assert hasattr(canvas, "_codes")
        assert isinstance(canvas._codes, list)

    def test_showpage_increments_page_count(self, tmp_path):
        """Test that showPage properly adds to page codes."""
        output_path = tmp_path / "test_showpage.pdf"
        ...
        story.append(PageBreak())
        story.append(Paragraph("Second Page", styles["Normal"]))
...
class TestPDFReportTemplate:
    ...
    def test_generate_report_large_content(self, tmp_path):
        ...
        styles = getSampleStyleSheet()
        elements = [
            Paragraph(f"Content Line {i}", styles["Normal"]) for i in range(100)
        ]

...
session_id_strategy = st.text(min_size=1, max_size=50, alphabet=st.characters(
    whitelist_categories=("Lu", "Ll", "Nd"),
    whitelist_characters="-_"
))

field_id_strategy = st.text(min_size=1, max_size=30, alphabet=st.characters(
    whitelist_categories=("Lu", "Ll", "Nd"),
    whitelist_characters="_"
))

button_id_strategy = st.text(min_size=1, max_size=30, alphabet=st.characters(
    whitelist_categories=("Lu", "Ll", "Nd"),
    whitelist_characters="_"
))
...
phase_strategy = st.one_of(
    st.none(),
    st.sampled_from(["Phase_1", "Phase_2", "Phase_3", "Phase_4"])
)
    ...
    def test_property_34_comprehensive_event_logging_button_click(
        ...
        **Validates: Requirements 38.1, 38.2**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert session_id in log_content, f"Session ID {session_id} should be in log"
            
            # Verify event type is in log
            assert "button_click" in log_content, "Event type 'button_click' should be in log"
            
            # Verify button_id is in log (may be JSON-escaped)
            # Check both raw and JSON-escaped versions
            button_id_found = button_id in log_content or json.dumps(button_id)[1:-1] in log_content
    ...
    def test_property_34_comprehensive_event_logging_input_change(
        ...
        **Validates: Requirements 38.1, 38.3**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Log input change
            logger.log_input_change(field_id, old_value, new_value, validation_result, session_id)
            ...
            # Verify event type is in log
            assert "input_change" in log_content, "Event type 'input_change' should be in log"
            
            # Verify field_id is in log (may be JSON-escaped)
            # Check both raw and JSON-escaped versions
            field_id_found = field_id in log_content or json.dumps(field_id)[1:-1] in log_content
    ...
    def test_property_34_comprehensive_event_logging_calculation(
        ...
        **Validates: Requirements 38.1, 38.4**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Verify event type is in log
            assert "calculation" in log_content, "Event type 'calculation' should be in log"
            
            # Verify calc_type is in log (may be JSON-escaped)
            # Check both raw and JSON-escaped versions
            calc_type_found = calc_type in log_content or json.dumps(calc_type)[1:-1] in log_content
            assert calc_type_found, f"Calculation type {calc_type} should be in log"
            
            # Verify engine_hash is in log
            assert engine_hash in log_content, f"Engine hash {engine_hash} should be in log"
    ...
    def test_property_34_comprehensive_event_logging_phase_transition(
        ...
        **Validates: Requirements 38.1, 38.6**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert source_phase in log_content, f"Source phase {source_phase} should be in log"
            
            # Verify dest_phase is in log
            assert dest_phase in log_content, f"Destination phase {dest_phase} should be in log"

    @given(
        method=st.sampled_from(["Logarithmic", "Box-Cox", "Yeo-Johnson", "Non-Parametric"]),
    ...
    def test_property_34_comprehensive_event_logging_method_lock(
        ...
        **Validates: Requirements 38.1, 38.7**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Verify event type is in log
            assert "method_lock" in log_content, "Event type 'method_lock' should be in log"
        ...
        **Validates: Requirements 38.11, 38.12, 38.16**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for i, line in enumerate(log_lines):
                ...
                timestamp, level, logged_session_id, event_type, context_json = match.groups()
                
                # Verify timestamp is ISO 8601 format
                assert "T" in timestamp, f"Timestamp should contain 'T' separator: {timestamp}"
                ...
                try:
                    ...
                    raise AssertionError(f"Context is not valid JSON: {context_json}") from e
        ...
        **Validates: Requirements 38.17**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for i, line in enumerate(log_lines):
                ...
                button_id_found = button_id in line or json.dumps(button_id)[1:-1] in line

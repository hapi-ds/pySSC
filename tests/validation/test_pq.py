...
def test_module_a_complete_workflow(page_with_app: Page):
    ...
    # Input confidence
    confidence_input = page.locator('input[aria-label*="Confidence"], input[placeholder*="Confidence"]').first
    confidence_input.fill("95")
    
    # Input reliability
    reliability_input = page.locator('input[aria-label*="Reliability"], input[placeholder*="Reliability"]').first
...
def test_module_a_report_generation(page_with_app: Page):
    ...
    allowable_failures_input = page.locator('input[aria-label*="Allowable"], input[placeholder*="Allowable"]').first
    ...
    report_button = page.locator('button:has-text("Generate"), button:has-text("Report")').first
    ...
    assert download.suggested_filename.endswith('.pdf'), (
        "Report should be a PDF file"
    )


@pytest.mark.pq
@pytest.mark.urs("33.3")
def test_module_v_complete_workflow(page_with_app: Page):
    ...
    two_sided_radio = page.locator('input[type="radio"][value*="Two"], label:has-text("Two-Sided")').first
    two_sided_radio.click()
    
    # Input LSL and USL
    lsl_input = page.locator('input[aria-label*="LSL"], input[placeholder*="LSL"]').first
    lsl_input.fill("8.0")
    
    usl_input = page.locator('input[aria-label*="USL"], input[placeholder*="USL"]').first
    usl_input.fill("16.0")
    ...
    # Input pilot data
    pilot_data = "10.0, 10.5, 11.0, 11.5, 12.0, 12.5, 13.0, 13.5, 14.0, 14.5, 15.0, 15.5"
    pilot_textarea = page.locator('textarea[aria-label*="Pilot"], textarea[placeholder*="data"]').first
    ...
    process_button = page.locator('button:has-text("Process"), button:has-text("Normality")').first
    ...
    calculate_n_button = page.locator('button:has-text("Calculate"), button:has-text("Sample Size")').first
    expect(calculate_n_button).to_be_enabled(timeout=3000)
    calculate_n_button.click()
    page.wait_for_timeout(2000)
    
    # Verify Phase 3 results appear
    phase3_results = page.locator('text=/Required Sample Size|k_margin|k_factor/i').first
    ...
    final_textarea = page.locator('textarea[aria-label*="Final"], textarea[placeholder*="final"]').first
    final_textarea.fill(final_data)
    
    tolerance_button = page.locator('button:has-text("Tolerance"), button:has-text("Calculate")').first
...
def test_concurrent_user_sessions(page: Page, base_url: str):
    ...
    try:
        ...
        conf1 = page1.locator('input[aria-label*="Confidence"], input[placeholder*="Confidence"]').first
        conf1.fill("95")
        rel1 = page1.locator('input[aria-label*="Reliability"], input[placeholder*="Reliability"]').first
        rel1.fill("95")
        
        # User 2: Input C=99%, R=95%, c=0 (different values)
        conf2 = page2.locator('input[aria-label*="Confidence"], input[placeholder*="Confidence"]').first
        conf2.fill("99")
        rel2 = page2.locator('input[aria-label*="Reliability"], input[placeholder*="Reliability"]').first
        rel2.fill("95")
...
@pytest.mark.pq
@pytest.mark.urs("33.3", "24.1", "24.2", "24.3", "24.4")
def test_module_v_sequential_workflow_enforcement(page_with_app: Page):
    ...
    phase2_button = page.locator('button:has-text("Process"), button:has-text("Normality")').first
    
    # Check if button is disabled (may need to check aria-disabled or disabled attribute)
    is_disabled = phase2_button.is_disabled() or phase2_button.get_attribute("disabled") is not None
...
def test_validation_state_display(page_with_app: Page):
    ...
    has_validation_indicator = (
        "validated" in page_content or 
        "validation" in page_content or
        "hash" in page_content
    )
...
@pytest.mark.pq
@pytest.mark.urs("26.1", "26.2", "26.3")
def test_tooltips_present(page_with_app: Page):
    ...
    confidence_field = page.locator('input[aria-label*="Confidence"], input[placeholder*="Confidence"]').first
    
    # Check for tooltip mechanisms (title attribute, aria-describedby, or info icons)
    has_tooltip = (
        confidence_field.get_attribute("title") is not None or
        confidence_field.get_attribute("aria-describedby") is not None or
        page.locator('text="?", [aria-label*="help"], [aria-label*="info"]').count() > 0
    )
...
def test_phase_invalidation_on_input_change(page_with_app: Page):
    ...
    has_phase1_results = "Q1" in content_after_phase1 or "IQR" in content_after_phase1
...
@pytest.mark.pq
@pytest.mark.urs("25.1", "25.2", "25.3", "25.4")
def test_method_transparency_display(page_with_app: Page):
    ...
    has_method_display = any(indicator in page_content for indicator in method_indicators)
